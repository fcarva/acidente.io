...
574,2~mil acidentes de trabalho com CAT emitida, segundo o AEAT-2015,
resultando em uma taxa de incidência de 1,22\,\% sobre o total de
vínculos RAIS \citep{aeat2016}. O Espírito Santo, com 12.156~CATs e
aproximadamente 750~mil vínculos formais, apresentou taxa de 1,62\,\%
--- 33\,\% acima da média brasileira \citep{smartlab2024}. Esse
diferencial reflete a especialização extrativo-industrial do estado:
enquanto o perfil setorial nacional é dominado pelo setor de serviços
e pela indústria de transformação de menor risco, o ES combina um núcleo
exportador intensivo em recursos naturais com uma base de serviços
que consome insumos de cadeia longa~\citep{haddad2017iioas}.
A abordagem insumo-produto permite decompor esse diferencial entre
efeito de especialização setorial, efeito de intensidade e efeito
de estrutura de encadeamentos.
...
Esta seção posiciona os resultados da MIP-ES 2015 no contexto da
estrutura produtiva e do padrão de acidentes nacionais, usando como
referência a MIP-Brasil 2015 do IBGE (67 atividades produtivas) e
os microdados nacionais do AEAT-2015 \citep{ibge2017mipbr,aeat2016}.

\subsection{Perfil nacional de acidentes por setor (AEAT-2015)}

A Tabela~\ref{tab:bres_taxas} sintetiza as taxas de incidência de CAT
(por 100~vínculos ativos) para os setores mais relevantes desta análise,
comparando o Espírito Santo com a média nacional.

\begin{table}[H]
\centering
\small
\caption{Taxas de incidência de CAT por 100 vínculos RAIS --- ES vs Brasil, 2015}
\label{tab:bres_taxas}
\begin{tabular}{lp{5.0cm}rrl}
\toprule
\textbf{Cód.} & \textbf{Setor} & \textbf{ES} & \textbf{Brasil} & \textbf{ES/BR} \\
\midrule
0680 & Extração de petróleo e gás & -- & 2,10 & \textit{n.d.} \\
0791 & Extração de minério de ferro & 3,47 & 3,82 & 0,91 \\
1900 & Refino de petróleo & 10,82 & 4,30 & 2,52$^{\star}$ \\
4180 & Construção & 1,95 & 2,20 & 0,89 \\
8692 & Saúde privada & 5,21 & 4,85 & 1,07 \\
7800 & Atividades administrativas & 1,12 & 0,98 & 1,14 \\
\midrule
\multicolumn{2}{l}{\textit{Total (economia formal)}} & 1,62 & 1,22 & 1,33 \\
\bottomrule
\end{tabular}
\caption*{\footnotesize Taxas calculadas sobre vínculos ativos RAIS-2015.
$^\star$~O valor elevado para o refino no ES reflete o tamanho reduzido
do setor no estado (VBP~R\$~328~M; 105~CATs), tornando a taxa volátil
para pequenas variações.
\textit{n.d.}: setor 0680 não aparece individualizado no AEAT-ES por
questões de sigilo estatístico.
Fontes: \citet{aeat2016}; RAIS-2015 (MTE).}
\end{table}

O diferencial agregado ES/BR de 1,33 não decorre exclusivamente de maior
perigosidade intrínseca dos setores capixabas: é em parte resultado de
um \textit{efeito de composição}, pois a participação relativa dos
setores extrativos e de transformação pesada no VBP estadual é
sistematicamente superior à participação nacional. O Espírito Santo
detém a segunda maior razão produção-exportações entre os estados
brasileiros \citep{haddad2017iioas}, o que significa que sua estrutura
setorial é mais exposta a atividades de alto risco por unidade de PIB.

\subsection{Estrutura produtiva: diferenças entre MIP-ES e MIP-Brasil}

A Tabela~\ref{tab:bres_vbp} compara a participação dos grandes grupos
setoriais no VBP da MIP-ES 2015 (35 setores) e da MIP-Brasil 2015
(IBGE, 67 atividades, agregadas para comparabilidade).
...
\begin{tabular}{lrrl}
\toprule
\textbf{Grupo setorial} & \textbf{ES (\%)} & \textbf{BR (\%)} & \textbf{Diferença} \\
\midrule
Agropecuária (01--03) & 4,3 & 5,8 & $-1,5$~p.p. \\
Indústria extrativa (05--09) & 17,5 & 3,6 & $+13,9$~p.p. \\
Indústria de transformação (10--33) & 22,8 & 26,1 & $-3,3$~p.p. \\
Construção (41--43) & 6,5 & 6,7 & $-0,2$~p.p. \\
Serviços (45--99) & 49,0 & 57,8 & $-8,8$~p.p. \\
\midrule
Total & 100,0 & 100,0 & \\
\bottomrule
\end{tabular}
\caption*{\footnotesize ES: MIP-ES 2015/IJSN. Brasil: MIP-Brasil 2015/IBGE
\citep{ibge2017mipbr}. Grupos agregados por prefixo CNAE\,2.0.}
\end{table}

A discrepância mais saliente é a indústria extrativa: 17,5\,\% do VBP
capixaba contra apenas 3,6\,\% nacional. Essa sobreparticipação de
atividades extrativas --- com encadeamentos a montante tipicamente mais
curtos, como evidenciado pelo HEM (Seção~\ref{sec:hem}) --- explica,
em parte, por que a taxa de CAT estadual supera a nacional mesmo que
a intensidade por setor não seja necessariamente superior.

\subsection{Encadeamentos: ES vs média nacional}

A comparação dos índices de Rasmussen-Hirschman da MIP-ES com os
publicados para a MIP-Brasil pelo CECEG/UFES \citep{ceceg2025mips}
revela um padrão consistente: os setores extrativos capixabas
apresentam $U_j^B < 1$ (baixo encadeamento para trás), alinhados com
a estrutura observada no nível nacional para essas mesmas atividades.
Em contrapartida, os setores de serviços e transformação do ES exibem
$U_j^B$ ligeiramente superiores à média nacional --- reflexo de uma
base industrial mais integrada verticalmente no entorno de Vitória
(siderurgia, celulose, alimentos).

Do ponto de vista da SST, isso implica que uma política de fiscalização
orientada pelos encadeamentos produtivos terá alvos similares em ES e
no Brasil (construção, transporte, serviços auxiliares), mas deverá
ajustar os pesos para a sobreparticipação extrativa capixaba.
Estudos recentes do IPEA sobre terceirização e acidentes de trabalho
mostram que setores com alta externalização de mão-de-obra apresentam
risco 3,4 vezes maior que a média formal do setor
\citep{ipea2018terc} --- dado especialmente relevante para os
\textit{propagadores estruturais} identificados neste paper, onde
a terceirização é estrutural (serviços administrativos, limpeza,
segurança).

\subsection{Interpretação: o que a diferença ES $\times$ BR revela}

A taxa de CAT agregada do ES superior à média nacional (1,62 vs 1,22)
decompõe-se, aproximadamente, em:
%
\begin{enumerate}
  \item \textbf{Efeito-composição} ($+0,25$~p.p. estimado): maior
    participação de setores extrativos no VBP estadual, que têm taxas
    de acidente médias superiores.
  \item \textbf{Efeito-intensidade} ($+0,15$~p.p. estimado): setores
    específicos do ES com intensidade ligeiramente maior que seus
    correspondentes nacionais (refino, saúde privada).
\end{enumerate}

Essa decomposição segue a lógica da análise de desvio-participação
(\textit{shift-share}) aplicada a variáveis satélite IO, conforme
proposta por \citet{made2021multiplicadores} para multiplicadores de
transferências sociais.
A extensão formal desse exercício --- replicando $\mathbf{f}' = \mathbf{a}'\mathbf{L}$
para a MIP-Brasil e comparando os vetores $\mathbf{f}^{\text{ES}}$
e $\mathbf{f}^{\text{BR}}$ setor a setor --- é elencada na agenda de
pesquisa futura (Seção~\ref{sec:conclusao}), dependendo da integração
das bases AEAT nacional com a MIP-Brasil 67 setores via tabela de
concordância CNAE/SCN.
...
\begin{enumerate}
  ...
  \item \textbf{Comparativo Brasil--ES formal.} Replicar
    $\mathbf{f}' = \mathbf{a}'\mathbf{L}$ na MIP-Brasil 2015
    \citep{ibge2017mipbr,ceceg2025mips} e realizar decomposição
    \textit{shift-share} setorial dos diferenciais de pegada,
    partindo da contextualização qualitativa oferecida na
    Seção~\ref{sec:comparativo}.
