...
def _sec_dados() -> str:
    ...
    \textit{lacuna normativa} setorialmente estruturada. A análise Monte
    Carlo com $\sigma_{\log}=0{,}20$ independente entre setores constitui,
    portanto, um limite inferior da incerteza real: uma perturbação
    metodologicamente mais precisa exigiria covariâncias positivas entre
    informalidade setorial e a magnitude do choque em $a_j$. A robustez
    a perturbações assimétricas setorialmente estruturadas permanece como
    extensão metodológica para trabalho futuro (Seção~\ref{sec:conclusao}).
...
def _sec_conclusao() -> str:
    ...
    A esses dois deslocamentos soma-se um terceiro, normativo: a lacuna
    institucional do sistema CAT/RGPS invisibiliza sistematicamente os
    trabalhadores mais expostos, concentrando atenção regulatória nos
    setores que já notificam bem~\citep{hitzig2020normative}.
    ...
    \begin{enumerate}
      ...
      \item \textbf{Responsabilidade regulatória por encadeamento.}
        ...
        quadrantes transmitem risco gerado a montante sem serem o alvo
        natural da regulação. Um arcabouço de SST informado pela análise IO
        permitiria: (a) exigir \textit{due diligence} de SST na cadeia de
        fornecimento dos setores-chave de risco; (b) rastrear o passivo de
        acidentes nos contratos de terceirização que conectam propagadores
        estruturais a setores de alta intensidade. Esse desenho é análogo
        ao que \citet{made2021multiplicadores} propõem para políticas de
        renda: o multiplicador identifica onde injetar o instrumento para
        maximizar o alcance sistêmico.

      \item \textbf{Federalismo fiscal do risco ocupacional.}
        O deslocamento geográfico revela uma externalidade fiscal: o
        Espírito Santo suporta o ônus sanitário e previdenciário do risco
        ocupacional dos setores extrativos (via SUS, RGPS-AT, DPVAT e
        afastamentos), mas o benefício econômico é capturado por
        exportadores e acionistas fora do estado. O HEM quantifica essa
        assimetria: o petróleo e o minério de ferro, com 17\% do VBP
        capixaba, respondem por menos de 3\% da pegada estrutural de
        acidentes intra-estadual. Uma agenda de equidade federativa em
        SST encontra nessa assimetria sua justificativa econômica mais
        precisa.
    \end{enumerate}
